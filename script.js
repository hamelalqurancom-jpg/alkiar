document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initFirebaseSync() {
        ...
        if (syncStatusUI) syncStatusUI.innerText = 'جاري الاتصال بالسحابة...';
    ...
    async function syncToFirestoreBackground() {
        if (!db) return;
        const syncStatusUI = document.getElementById('sync-status');
        const syncIndUI = document.getElementById('sync-indicator');
        
        if (syncStatusUI) syncStatusUI.innerText = 'جاري مزامنة السحابة...';
        ...
        try {
            ...
            for (const col of Object.keys(lastSyncedData)) {
                ...
                for (const item of appData[col]) {
                    ...
                    if (strItem !== lastStr) {
                        hasChanges = true;
                        promises.push(db.collection(col).doc(String(item.id)).set(item, { merge: true }));
            ...
            if (syncStatusUI) syncStatusUI.innerText = 'تمت المزامنة مع السحابة';
            if (syncIndUI) syncIndUI.style.background = '#10b981';
        } catch (err) {
            console.error("Firebase Sync error:", err);
            if (syncStatusUI) syncStatusUI.innerText = 'خطأ في رفع البيانات للسحابة!';
            if (syncIndUI) syncIndUI.style.background = '#e11d48';
        }
    }

    setTimeout(() => { if (db) initFirebaseSync(); }, 1500);
    ...
    let selectedBulkCases = []; // Track cases selected for bulk distribution in expenses page
    ...
    async function updateDataInFile() {
        ...
        try {
            ...
            if (syncIndicator) syncIndicator.style.background = '#1d4ed8';
        } catch (err) {
            console.error('Auto-save failed', err);
            if (syncStatus) syncStatus.innerText = 'خطأ في الحفظ!';
            if (syncIndicator) syncIndicator.style.background = '#e11d48';
        }
    }
    ...
    // Delegate settings button if it exists
    document.addEventListener('click', async (e) => {
        if (e.target && (e.target.id === 'link-folder-btn-settings' || e.target.closest('#link-folder-btn-settings'))) {
            try {
                directoryHandle = await window.showDirectoryPicker();
                await loadDataFromFile();
                renderPage('settings'); // Refresh settings UI
            } catch (err) {
                console.error('Folder selection cancelled', err);
            }
        }
    ...
    function saveData(writeToFile = true) {
        ...
        syncTimeout = setTimeout(syncToFirestoreBackground, 2000);
    }
        ...
        item.addEventListener('click', () => {
            ...
            if (page === 'expenses' && !window.expensesUnlocked) {
                ...
                if (pass === appData.systemSettings.expensesPassword) {
                    ...
                    // Switch back to dashboard or active page in UI
    ...
    function renderPage(page, contextId = null) {
